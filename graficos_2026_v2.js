...
function parseDataRelativa(valor, baseRef) {
 ...
 function atualizarAtualizadoEm(dados) {
  ...
  function atualizarGraficoCriadosMes(lista) {
    ...
    let __semDataCriacao = 0;
    lista.forEach(d => {
      const dt = parseDataFlex(d["Criado_dt"]); // ✅ somente data de criação
      if (dt) contagem[dt.getMonth()]++;
      else __semDataCriacao++;
    });
    if (__semDataCriacao) console.warn("SRs sem Criado_dt válido (não entram em Abertos por mês):", __semDataCriacao);
